#if !defined(MDNS_LISTENER)

#define MDNS_LISTENER
...
{
	static const char * check_(int domain)
	{
		if (domain == AF_INET) return "AF_INET";
		if (domain == AF_INET6) return "AF_INET6";
		ERROR("Unsupported domain (%d)", domain);
		return nullptr; // stop compiler warning
	}

	// Create a socket of the specified family, enabling various useful options.
	static int Create(int domain)
	{
		const int on = 1;

		auto dstr = check_(domain);

		// Ensure whatever port/address the socket eventually binds to can be
		// reused quickly.

		int s = socket(domain, SOCK_DGRAM, 0);
		if (s < 0) ERROR("socket(%s)", dstr);

		if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))<0) {
			ERROR("setsockopt(%s,SO_REUSEADDR)", dstr);
		}

		if (setsockopt(s, SOL_SOCKET, SO_REUSEPORT, &on, sizeof(on))<0) {
			ERROR("setsockopt(%s, SO_REUSEPORT)", dstr);
		}
		...
		int proto = (domain == AF_INET6) ? IPPROTO_IPV6 : IPPROTO_IP;
		int option = (domain == AF_INET6) ? IPV6_RECVPKTINFO : IP_PKTINFO;

		if (setsockopt(s, proto, option, &on, sizeof(on)) < 0) {
			ERROR("setsockopt(%s,PKTINFO)", dstr);
		}
	...
	static int CreateAndBind(int family, int port, const struct ifaddrs* ifa = nullptr)
	{
		...
		// Create socket with appropriate options set
